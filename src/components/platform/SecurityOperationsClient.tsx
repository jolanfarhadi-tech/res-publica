...
export function SecurityOperationsClient({ locale }: { locale: Locale }) {
  ...
  const {
    incidents, observations, claims, correlations,
    defensiveSignals, defensiveActions, defensiveActionEvents,
  } = state.payload;
  return <div className="space-y-10">
    ...
    <section aria-labelledby="security-responses"><h2 id="security-responses" className="text-3xl">{copy.responses}</h2><p className="mt-3 max-w-3xl text-sm leading-relaxed text-muted">{copy.responseBoundary}</p><ul className="mt-5 space-y-4">{defensiveActions.map((item) => { const states = defensiveActionEvents.filter((event) => event.actionId === item.id).map((event) => event.state); return <li key={item.id} className="rounded-2xl border border-border bg-surface p-6"><p className="civic-label">{item.policyId} · {item.evidenceLevel}</p><h3 className="mt-2 text-xl">{item.action}</h3><dl className="mt-4 grid gap-3 text-sm md:grid-cols-2"><div><dt className="text-muted">{copy.actionClass}</dt><dd>{item.actionClass}</dd></div><div><dt className="text-muted">{copy.effectState}</dt><dd>{states.join(" → ")}</dd></div><div><dt className="text-muted">{copy.target}</dt><dd>{item.targetAsset} · {item.targetScope}</dd></div><div><dt className="text-muted">{copy.reversibility}</dt><dd>{item.reversibility}</dd></div></dl><p className="mt-4 text-sm text-muted">{item.rationale}</p></li>; })}</ul><p className="mt-4 text-sm text-muted">{copy.loopEvidence}: {defensiveSignals.length}</p></section>
  </div>;
}
