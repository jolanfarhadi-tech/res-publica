...
export async function POST(request: Request) {
  const ctx = await context(request); if ("response" in ctx) return ctx.response;
  const parsed = grantSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return Response.json({ error: "invalid_request" }, { status: 400 });
  try {
    const grant = await grantEditorialRole(ctx.runtime.db, ctx.actor, { ...parsed.data,
...
export async function DELETE(request: Request) {
  ...
  if (!parsed.success) return Response.json({ error: "invalid_request" }, { status: 400 });
  try { return Response.json({ grant: await revokeEditorialRole(ctx.runtime.db, ctx.actor, parsed.data) }); }
  catch (error) { return editorialError(error); }
}
