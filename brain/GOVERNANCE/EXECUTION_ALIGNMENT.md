...
| 32: Validation Engine | Naming overlap only: Review & Validation Agent (`ADR-004`) — a reviewer role, not a product engine | No (different kind of thing) | **Future Proposal (not approved)** | N/A | New ADR required | Clarify this is not the same as the existing Agent role |
