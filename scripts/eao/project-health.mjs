...
// small inline TODO-heading count that reuses the existing shared forEachLine/
...
export function findTodos(root = process.cwd()) {
  ...
  // A TODO is actionable only when it is declared as a Markdown section
  // heading. Prose that discusses TODO detection, historical TODOs, or the
  // category name is documentation, not an outstanding task marker.
  ...
  for (const file of findMarkdownFiles(root)) {
    ...
    forEachLine(file, (lineText, lineNum) => {
      ...
      if (!fenceMarker && /^#{1,6}\s+TODO\b/.test(trimmed)) {
...
export function computeProjectHealth(root = process.cwd()) {
  ...
  return {
    ...
    technicalDebtIndicators: {
      ...
      note: "Count of Markdown TODO section headings across all scanned Markdown files.",
    },
...
export function renderProjectHealthMarkdown(h) {
  ...
  lines.push("");
  lines.push("_Generated by scripts/eao/project-health.mjs - composes Repository Health, Broken Link Detection, Terminology Drift, and Dependency Analysis. Introduces no new detection logic beyond a reused TODO-heading count._");
  lines.push("");
