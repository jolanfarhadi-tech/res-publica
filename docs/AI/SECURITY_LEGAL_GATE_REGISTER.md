...
| Clean recovery and current restore evidence | Repeatable synthetic backup→restore verifies the current 25/102 schema, audit fixture and revoked access; the Neon verifier is read-only and no longer executes a mutable remote CLI | Current isolated Neon drill, RPO/RTO, independent backup/access, owners and dual-control cutover required | Current provider-recovery and destructive-cutover assurance |
| Technical attribution operations | Append-only A–D evidence/claims/correlations, daily rotating HMAC identifiers, exact MFA authority, separation of duties, canonical audit and protected multilingual read console are implemented; Level E, provider enrichment and source scanning are absent | Incident-processing purpose/legal basis, retention/legal hold, named operators, correlation secret/grants, Production migration and any provider-source approval required | Real security-incident intake and provider-enriched attribution operations |
