...
| Zero-day process and egress isolation | Exact capability/write-scope quarantine and Research fail-closed override are implemented; the modular monolith still shares compute, main DB credential and environment namespace, and Vercel network egress policy is unverified | Deployment architecture and provider controls required | Claim of service-level blast-radius isolation |
| DDoS and application resource-exhaustion controls | Endpoint-specific PostgreSQL limits, declared-body budgets, bounded pagination/query inputs and a degradation runbook are implemented; Vercel Firewall/WAF, bot/challenge, stream limits, security-log export and alert ownership are not verified | Provider/operations evidence required | Claim of complete edge and volumetric protection |
| Clean recovery and current restore evidence | Repeatable synthetic backup→restore verifies the current 26/105 schema, audit fixture and revoked access; the Neon verifier is read-only and no longer executes a mutable remote CLI | Current isolated Neon drill, RPO/RTO, independent backup/access, owners and dual-control cutover required | Current provider-recovery and destructive-cutover assurance |
| Technical attribution and defensive correspondence operations | Append-only A–D evidence/claims/correlations, rotating HMAC identifiers, ordered Loops 1–5, bounded A→A′ decisions/effects/rollback, exact MFA authority, separation of duties, canonical audit and protected multilingual console are implemented; Level E, provider enrichment, source scanning and autonomous high-impact/provider actions are absent | Incident-processing purpose/legal basis, retention/legal hold, named operators/reviewers, correlation secret/grants, Production migrations 0024–0025 and any provider-source/action approval required | Real security-incident intake, provider-enriched attribution or provider-side containment |
...
| Public API partner platform | Anonymous V1 exposes only allowlisted grounded public Content Graph DTOs; partner accounts, keys, agreements, quotas, embeds and Event integration are absent | Partner/legal/privacy/security/operations approval required | Institutional partner access and integration |
