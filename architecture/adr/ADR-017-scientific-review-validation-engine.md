...
## Addendum (Critical Architecture Patch): `hearing_validated` renamed to `hearing_documented`

An architecture consistency review found that the Level 2 state name `hearing_validated` (as originally adopted above) contradicted a subsequently-established principle: Structured Hearings document and contextualize an account through multiple perspectives, but do not validate it — Scientific Review is the first formal validation process in the HARM lifecycle (see `docs/source/methodology/STRUCTURED_HEARINGS.md`'s "Reflection ≠ Validation" clarification and `docs/source/methodology/SCIENTIFIC_REVIEW.md`).

The state is renamed `hearing_documented` in `03_ANNEX_BLOCKCHAIN_CIVIC_CONTRIBUTION_ARCHITECTURE.md` §7's state machine and in all citing documents (`BASIC_VALIDATION_FRAMEWORK.md`, `SCIENTIFIC_REVIEW.md`, `brain/ARCHITECTURE/CIVIC_INTELLIGENCE_KNOWLEDGE_GRAPH_RELATIONSHIP.md`). This is a label change only — the state's position in the machine (`expert_reviewed → hearing_documented → codex_validated`, Level 2 pass, with the same Level 3 return and Level 4/Ethics Board veto transitions) is unchanged. Nothing above this addendum is rewritten; the original Context/Decision text is left as the historical record of the state machine's initial adoption, including its original name.
