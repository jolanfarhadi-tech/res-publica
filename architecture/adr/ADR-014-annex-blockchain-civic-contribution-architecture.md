...
This ADR's original lifecycle (§Decision) placed the Responsibility Dashboard at the end, as primarily a downstream consumer of the Contribution & Impact Framework's output. `ADR-016` corrects this: the Dashboard's **primary** process position is upstream, Innovation 3, before Annex generation, as the prioritization instrument producing a Top Priority Selection that feeds Annex Deepening. The downstream-consumption role described here is retained as **secondary**, not primary. See `ADR-016` and `brain/FOUNDATION/03_ANNEX_BLOCKCHAIN_CIVIC_CONTRIBUTION_ARCHITECTURE.md` §2/§5 (as amended) for the corrected lifecycle. This original text is preserved unedited above, per the Constitution's amendment discipline (`brain/00_constitution/00_constitution.md` §17) — the correction is appended, not silently rewritten in place.
